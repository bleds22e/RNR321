...
We are using a document called an [RMarkdown]{.underline} file (ending is `.Rmd`). In RMarkdown files, we can type regular text—just like this—or we can type code and see the output.

With plain text, you can type normally in the document.

To type code that we can use, we need to type it in a "code chunk."

```{r}
# This is a code chunk! 
# The {r} indicates that this chunk only "understands" the R language
# Always start typing on the line below the {r} 
```

## Running Code

How do you "run" code? Running or executing code effectively means that you are sending a line of code to the console for R to interpret it, follow the instructions, and return the output.

If you are working on the console (uncommon in this class), you hit `Enter`.

When working in the source panel (like this document), we will be typing all of our code into code chunks.

There are a few different ways to run lines of code written in code chunks:

- To run one line of code at a time, put your cursor on the line you want to run and either click the "Run" button in the upper right-hand corner of the source panel or hold down `Ctrl` + `Enter` (`Cmd` + `Enter` on Macs).
- To run a couple lines of code, highlight them and do the same thing as above.
- To run every line in a code chunk, click on the green arrow on the right side of the code chunk.

## Using R as a calculator

We can perform mathematical calculations using R.
...
You can add notes or information into code chunks using `#`. This is called adding comments.

```{r}
# we can add comments to our code by using the # sign
# R doesn't read anything past a #
# you can put one at the beginning of a line or after some code
...
We can also do basic math in the console.
...
Try multiplying 5 and 3 in the console.

# Assigning Objects

Assignments are key to almost everything we do in R. We create permanent [objects]{.underline} in R by assigning data to a name, meaning we can reference these objects later.
...
2.  Each new object will show up in your environment (upper right panel). This is particularly helpful to check if our code ran successfully or to confirm that the object represents what we *think* it represents.
...
```{r}
height * 2      # height does not change
age - 20        # age does not change

# if we use the assignment operator, the object will be overwritten
age <- age - 20 # age object changes
...
Functions are always followed by parentheses. Anything you type into the parentheses are called [arguments]{.underline}.

| `function_name(argument1, argument2,...)`

Arguments allow us to give the function additional information about how we want it to perform its task.
...
[Vectors]{.underline} are the most common and basic data type in R. They make up most of the other data types we will work with in R.

They are composed of series of values (called [elements]{.underline}), which can either be numbers, characters, or other types of data.
...
```{r}
# combine 3 vectors into 1 long vector
test_vec <- c(weight_g, animals, mammal) 
test_vec 

class(test_vec) 
# coerced everything into character (don't know how to make words numeric)
```
...
- by [index]{.underline} (position of the element)
- by [condition]{.underline} (meeting certain requirements)
...
```{r}
weight_g[2]       # second element only
weight_g[c(2,4)]  # second and fourth elements
weight_g[c(1:4)]  # first through the fourth elements (4 total)
```
...
sum(last3) # sum(vec[3:5]) also works!
```

If you've finished that task, try to get the same result using *conditional* sub-setting.

```{r}
# conditional subsetting statements can also be arguments in vectors
sum(vec[vec > 5])
```
