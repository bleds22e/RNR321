...
We want to estimate the total abundance of desert pocket mice in our population using the Lincoln-Peterson equation.
...
During this same trapping experiment, we also caught 12 Bailey's pocket mouse, *Chaetodipus baileyi*, on the first night and marked them. The second night, we caught another 12 Bailey's, 4 of which were previously marked.
...
5.  Using those values, use the Lincoln-Peterson estimator to estimate the population total of Bailey's pocket mouse.
...
*I don't expect you to know how this code is working!*
...
```{r}
tau_hat_estimates <- LP_estimator(p1 = 0.4, p_recap = 0.4)
```

Now, let's plot a histogram of our estimates.

```{r}
hist(tau_hat_estimates)
```
...
mean(tau_hat_estimates)          # mean of all tau-hat estimates

# On average, how biased are our estimates?
mean(tau_hat_estimates) - tau    # bias

# How precise are our estimates?
sd(tau_hat_estimates)            # precision
```
...
1A: Detection is *higher* than the demo 1B: Detection is *lower* than the demo

**Scenario 2: Detection Probability Changes with Behavior**

2A: Animals are "trap shy" 2B: Animals are "trap happy"
...
13. Compare the bias and precision of our total abundance estimates for the *demonstration* (the code we ran in between Questions 7 and 8), trap happy, and trap shy populations.

Write 2-3 sentences explaining the results. How do animals being trap happy or trap shy affect the accuracy of our estimate? Why is this happening? (2 points)
