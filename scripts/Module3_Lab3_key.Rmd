...
```{r}
beads <- read.csv("../../../../ebledsoe@arizona.edu - Google Drive/My Drive/UArizona/Teaching/RNR321/2024_spring/Labs/Distance_Data_Spring2024.csv")
```
...
```{r}
transect_length <- data.frame(group_id = 1:8,
                              length_m = mean(green$Transect_m))
```
...
area <- 15 * mean(beads$Transect_m) # 480 m^2
...
Maddie and I put out 150 green beads, so this is pretty good!
...
# Exponential model
d_hat <- backTransform(HNc, type = "state")@estimate  # 5719.2
d_hat
