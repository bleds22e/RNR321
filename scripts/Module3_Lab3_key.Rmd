...
```{r}
beads <- read.csv("../data_raw/Distance_Data_Spring2026.csv")
```
...
We can set "cut points" for which values to use to bin our distance data in our model; in our case, we want every half meter.

This line of code creates a vector that will have values every half meter (`by = 0.25`), starting at 0 and running through the value we set for truncation (`trunc`).
...
green_binned <- formatDistData(distData = green,       # data frame with the data points
                        distCol = "Distance_m",        # column that holds the distance values
                        transectNameCol = "GroupID",   # column that holds the transect groups
                        dist.breaks = distance_bins)   # vector of bin groupings for distances
green_binned
```
...
transect_length <- unique(green[ , c("GroupID", "Transect_m")]) 
transect_length
```
...
```{r}
# join binned dataframe with transect length dataframe
UMF_green <- unmarkedFrameDS(y = green_binned,        # binned data
                       survey = "line",             # type of survey conducted (line or point)
                       tlength = transect_length$Transect_m,  # column w/ each transect length
                       dist.breaks = distance_bins, # vector of how distances are binned
                       unitsIn = "m")        

UMF_green   # prepared data
```
...
esw <- integrate(gxhn, lower = 0, upper = 2.25, sigma = sigma)$value
esw
```
...
```{r}
# Our survey area (m) = [12 groups x (2*esw*L)]
area <- (12 * (2 * esw * 26.5))
area
...
I put out 150 green beads, so this is off by a bit! That's ok, though. 

We did *repeated* surveys in the same area and did not take that into account in our model. If we were going to accurately analyze our data, we would definitely need to do that.
