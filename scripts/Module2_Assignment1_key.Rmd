...
10. Run the line below to create the object heights. Use the `mean()` function to calculate the mean value of height.
...
11. This should yield an odd result caused by the NAs. To resolve this,
...
15. Create a character vector called `rhymes` that contains the following values: cat, rat, bat, hat 
```{r}
...
The remainder of the questions in this assignment lead you through the steps to calculate the coefficient of variance for two different species at the field site--the largest (NL) and the smalled (BA).

21. Using a similar structure as the code in question 21, create 2 different data frames: one called `NL` with rows that have "NL" in the species_id column and one called `BA` with only rows which have "BA" in the species_id column.
```{r}
...
24. Using the equation for calculating the coefficient of variance (CV), calculate the CV for each species.
```{r}
...
The CV, which standardizes measures of spread by the size of the mean, indicates that the NL species has a higher amount of variance in its measured weights than the BA species.
