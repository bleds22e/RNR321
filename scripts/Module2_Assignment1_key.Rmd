...
estimates of population paramerters and assess accuracy of estimates.
...
1.  Define the *sample* standard deviation of the mean in terms of our
    ...
    deviation, what it tells us, and what it represents).
...
(a) I recommend referencing the slide decks from class! Remember,
...
### Calculating The Uncertainty in our Estimate of the Mean
...
13. Calculate the SE of the estimate of the mean by taking the square
    root of the `rocks_var_ybar` value. Call this object
    `rocks_SE_ybar`.

```{r}
rocks_SE_ybar <- sqrt(rocks_var_ybar)
rocks_SE_ybar
```

### Estimating the Population (and Uncertainty of the Estimate)
...
# alternative equation is N^2 * rocks_var_ybar
rocks_var_total
```
...
-   confidence (uncertainty) is estimate of number of dark rocks total =
    `rocks_SE_total` or `rocks_var_total` (rocks or rocks squared,
    respectively)
-   density estimate = `rocks_mean` aka "dark rocks per 4x4 in\^2 or
    16in sq or "little square"
-   uncertainty in density estimate = `rocks_var_ybar` (rocks squared
    per little square) or `rocks_SE` (rocks per little square)
-   variation in density = `rocks_var` or `rocks_sd` (rocks squared or
    rocks, respectively)
