...
cactus <- read.csv("../output/all_cactus_data_sp2024.csv")
```
...
*Answer: The x-axis indicates groupings/bins of depth measurements for each cactus pad and the y-axis indicates the frequency with which measurements are in each group. While the range of the data is from about 0-2 the majority of values seem to fall between 0.2-0.6. The distribution is right-skewed.*
...
17. Interpret the results from Q15 and Q16. What do they tell us about the depth of cactus pads and the distribution of the data?

    *Answer: Opuntia engelmannii* seems to have wider pads on average than *O. ficus-indica*. The *O. ficus-indica* data has a higher standard deviation, meaning that there is more variation in the data points (they are less close to the mean than the *engelmannii* data points).
