...
source("scripts/TEAM library 1.7.R")
library(chron) 
library(reshape)
library (vegan)
library(plotrix)
...
team_data<-read.csv(file="data_raw/teamexample.csv", sep=",",h=T,stringsAsFactors=F)

### add Class, Order, Family attributes using the IUCN database

iucn.full<-read.csv("data_raw/IUCN.csv", sep=",",h=T)
...
write.table(camera_days, file="data_raw/camera_days_2009.txt",quote=F, sep="\t",row.names = F)
...
write.table(events_hh, file="data_raw/events_hh.txt",quote=F, sep="\t")
write.table(events_dd, file="data_raw/events_dd.txt",quote=F, sep="\t")

# cumulative events per species 
events_hh_species<-colSums(events_hh)
write.table(events_hh_species, file="data_raw/events_hh_species.txt", quote=F, sep="\t")

events_dd_species<-colSums(events_dd)
write.table(events_dd_species, file="data_raw/events_dd_species.txt",quote=F, sep="\t")

# cumulative events per camera sites
cameras<-rowSums(events_hh)
write.table(cameras, file="data_raw/events_species.txt",quote=F, sep="\t")
...
write.table(naive_occu_2009, file="data_raw/naive_occu_2009.txt",quote=F, sep="\t",row.names = F)


# accumulation curve  
accumulation<-acc.curve(data,2009.01)
write.table(accumulation, file="data_raw/accsp_2009.txt",quote=F, sep="\t")
...
write.table(activity_24h, file="data_raw/events_24hour_2009.txt",quote=F, sep="\t",row.names = F)
