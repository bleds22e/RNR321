...
1.  Plot the distribution of detection distances. Does is look how you expected?

```{r}
hist(saguaro$distance) 
```

2.  Create an object called `trunc` that sets the truncation distance to 20m to eliminate extreme observations.
...
3.  Use `unmarked` function `formatDistData` to count the number of detections in each distance bin for each transect. Save this as an object called `sag_data`.
...
4.  Using the `unique()` function, make a data frame with transect length (i.e., survey effort). Call this `transect_saguaro`. Make sure you are referencing the correct columns in the square brackets!

```{r}
transect_saguaro <- unique(saguaro[ , c("group", "length")])
transect_saguaro
```
...
*Answer:* 49.52 saguaros \<2m per hectare (39.58, 61.97); under because we could not account for the ones we missed during sampling
...
11. Plot the distribution of detection distances to get an idea of what they look like.

```{r}
hist(ocotillo$distance) 
```

12. This time around, set truncation distance to 25m; save this as an object called `trunc_oco`. Then create your distance vector (`distance_bins_oco`) and set the cut points at every 5 meters.
...
13. Use the function `formatDistData` to count the number of detection in each distance bin for each transect. Ignore the warning message. Remember to use the correct data and distance bins.
...
14. Make your data frame with the transect lengths. Call this `transect_oco`.

```{r}
transect_oco <- unique(ocotillo[ , c("group", "length")])
transect_oco
```
