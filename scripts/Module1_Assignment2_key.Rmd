...
**Grading note: I've copied these definitions from my slides. If their definitions are exactly the same, they have copied directly from the slides and not used their own words, so take 0.5 points off.**
...
9.  Run the line of code below. Based on the output, explain what the line of code does (\~ 1 sentence).

```{r}
snails[3:5]
```

*Answer*: Returns the third, fourth, and fifth elements from the snails vector
...
Here, I'm saving the cactus pad length column as a vector called `pad_length`. Once you run this code chunk, you should see `pad_length` vector in your environment.
...
12. Write a line of code that saves the 42nd value in the `pad_width` vector as an object called `answer`.
...
*Answer: Cactus pad lengths are longer, on average, than cactus pad widths. The lengths also have more variability in their values (higher st. deviation), meaning that the lengths are further away from the mean than the widths are.*
