...
```{r}
# 50 rows, each corresponding to a different plot
# 225 column, each for a different species
# head(BCI)
```
...
```{r}
BCI_observed <- BCI_richness$Species
BCI_observed

BCI_estimated <- BCI_richness$chao
BCI_estimated
```

Based on our estimated and observed richness values, how many species does the model think are present in the community but we have not detected?

```{r}
BCI_estimated - BCI_observed
```

### Plotting the Estimate

Let's add a horizontal line in our species accumulation curve plot to compare the asymptote (estimated species richness) against our curve (observed species richness).
...
```{r}
plot(BCI_sac, ylim = c(0, BCI_estimated + 5))
abline(h = BCI_estimated, col = "red")
```
