...
[`portalr`](https://weecology.github.io/portalr/)) along with the
`tidyverse`.
...
have the number of each species caught on each individual plot that
trapping session.
...
1.  First things first, we want to create an object for our sample size.
    ...
    object `n`, which is the number of sample units that make up our
    sample. To do this, use the `nrow()` function. This will be the same
    for both species.
...
10. What happens when we create 90% CI for the data? Calculate 90% CI
    and compare to the 95% CI. Which one is wider? Why? (3 points)
...
16. Compare the ranges of the 95% CI for the PP and the PB data. Which
    ...
    confidence intervals? Why do you think that is? (2 points)
