...
The data has columns with information about the trapping session and plots; the remaining columns with 2 letters each represent rodent species. The values in the columns indicate how many individuals of that species were caught in a given plot.
...
## Estimating the Density of *Chaetodipus pencicillatus* (PP)
...
As a reminder, when we are estimating a *density*, we are estimating the *mean* number of entities per sample unit.
...
3.  Create an object called `PP_var` which contains the sample variance. Hint: because this is the *sample* variance, we can use the `var()` function.
...
Reminder: in this assignment, we don't know what `N` is. This will (a) impact the version of the equations we use for calculating standard error and (b) make it impossible to calculate the population total for the site.

4.  Calculate the variance of the estimate of the mean for the PPs. Hint: there is no function in R that will do this for us, so you will need to use the objects you've assigned in the questions above and the equations we covered in class.
...
b.  Now, using `t90`, calculate the 90% CI. 
...
c.  Compare the 90% CI to the 95% CI. Which one is wider? Why would we expect that to be the case?

*Answer:*

## Estimating the Density of *Chaetodipus baileyi* (PB)
...
15. Create 95% CI for the PB data. You should already have the t-value as an object in your environment, so you don't need to go through calculating that again.
...
16. Compare the estimated densities of PPs and PBs. Which species is present in higher densities at the site?

*Answer*:

17. Now, compare the ranges of the 95% CI for the PP and the PB data. Which density estimate seems to be more accurate? How are you using to make that determination? 

    Taking a look back at the equation for calculating confidence intervals and the values for each species in that equation, what value is driving the difference in how wide the confidence intervals (AKA the range of the interval)?
