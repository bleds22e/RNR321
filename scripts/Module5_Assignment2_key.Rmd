...
8. Based on the target of these surveys (saguaros > 2m) and what you know about the assumptions of PCQ methods, do you think this estimate of density is a reliable estimate? Why or why not? (2 points)
  ...
  _Answer:_ yes; saguaros are inanimate, large (> 2m) are hard to miss so detection is probably close to 1. not sure about random distribution.

## Spatial Patterns 

Before you start working on this section, I _highly_ recommend that you click the little broom button over in the Environment panel (upper right-hand panel). Some of the objects you will create have the same names as the ones we used in the lab example and could wreak havoc on your calculations.

That said, the script that we used in lab today will be very helpful!

### T-square Sampling

First, let's create some data. Let's say these data represent Osage orange trees (_Maclura pomifera_) across a landscape.
...
Now we have 50 random points (`point`) which each have two associated measurements, `x` and `y`.

9. Determine whether the sum of `x^2` is greater, equal to, or less than the sum of 1/2`y^2`. What does this suggest to us? (2 points)
```{r}
sum(x_i^2)
sum(.5*y_i^2)
```

  _Answer:_ 1/2y^2 is bigger, suggesting clumped
...
11. Is the value of `C` consistent with what we determined in question 10?
...
14. Should we reject the null hypothesis? What spatial pattern do we determine this population to have? Explain your rationale (2 points)
  ...
  _Answer:_ p < alpha of 0.10, so reject null hypothesis--clumped distribution

### Plot-Based Estimates

Below are some data from Pacific silver fir (_Abies amabilis_) seedlings were counted on 104 10 x 10 cm plots.
```{r}
...
16. From what we've talked about in class, hypothesize which pattern of dispersion these data are likely to show: clumped, random, or uniform. Why? (2 points)
Hint: think about the mean and the variance.

  _Answer:_ variance is higher than mean, so suggests clumped
...
21. What is our spatial pattern type? Explain your rationale (2 points)
  ...
  _Answer:_ clumped because p > .90 so we reject null of random
...
22. Think critically about what the variance of sample data means. In your ownwords, explain why you would expect the variance for a species with a uniform distribution to be smaller than the variance for a species with a clumped distribution? (2 points)
  ...
  _Answer:_ each sample is likely to be very similar in a uniform distribution, so there will not be a lot of spread around the data. in clumped, you might have a bunch of samples with zeros or low numbers and a few with mid- or high numbers, creating a much higher spread in the data.