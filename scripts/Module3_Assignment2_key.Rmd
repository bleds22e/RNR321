...
```{r}
LP_estimator <- function(p1, p2, p_recap) {
  ...
  for(i in 1:nRuns) { 
    ...
    u2 <- rbinom(1, tau-c1, p2)  # no. marked individuals recaptured on survey occasion 2
...
Let's say that our detection probabilities (`p1` and `p2`) and recapture probability (`p_recap`) are consistent, always at 0.4.
...
```{r}
tau_hat_demo <- LP_estimator(p1 = 0.4, p2 = 0.4, p_recap = 0.4)
```
...
8.  Our detection probabilities (`p1` and `p2`) and recapture probability (`p_recap`) are consistent and *higher* than the demo: `p1` = 0.6, `p2` = 0.6, `p_recap` = 0.6 (2 points)

    a.  Run the simulation.

    ```{r}
    tau_hat_high <- LP_estimator(p1 = 0.6, p2 = 0.6, p_recap = 0.6)
    ```
...
9.  Our detection probabilities (`p1` and `p2`) and recapture probability (`p_recap`) are consistent and *lower* than the demo: `p1` = 0.2, `p2` = 0.2, `p_recap` = 0.2 (2 points)

    a.  Run the simulation.

    ```{r}
    tau_hat_low <- LP_estimator(p1 = 0.2, p2 = 0.2, p_recap = 0.2)
    ```
...
11. We have a population that is trap shy, meaning that the probability of recapture is lower than the probability of initial capture (`p > c`): `p1` = 0.4, `p2` = 0.4, `p_recap` = 0.2 (2 points)

    a.  Run the simulation.

    ```{r}
    tau_hat_trapshy <- LP_estimator(p1 = 0.4, p2 = 0.4, p_recap = 0.2)
    ```
...
12. We have a population that is trap happy, meaning that the probability of recapture is higher than the probability of initial capture (`p < c`): `p1` = 0.4, `p2` = 0.4, `p_recap` = 0.6 (2 points)

    a.  Run the simulation.

    ```{r}
    tau_hat_traphappy <- LP_estimator(p1 = 0.4, p2 = 0.4, p_recap = 0.6)
    ```
...
13. Compare the bias and precision of our total abundance estimates for the demo, trap happy, and trap shy populations. Write 2-3 sentences explaining the results. How do animals being trap happy or trap shy affect the accuracy of our estimate? Does this make sense to you? (2 points)
    ...
    * Students do not need to explain how this works, but I'm putting it here so WE know why, because I struggle with it every semester!
    
    The reason trap happy animals bias our estimate negatively (we underestimate) boils down to the LP equation. The assumption is the that proportion of marked individuals in the second capture event is the same as the marked individuals in the population. When animals are trap happy, we end up with more recaptures than "expected", meaning the proportion of marked individuals in the second capture is higher than the proportion of individuals in the population. The only way to get the proportions back to equal is the make the tau_hat estimate lower than it actually is. 
    Alternatively, we can think about the tau_hat = c1*c2 / r2 equations. If we are recapturing more individuals than we "should" be, we are dividing by a bigger number, which will ultimately make tau_hat lower.*
