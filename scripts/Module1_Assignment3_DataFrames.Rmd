...
2.  The answer to Q1 should yield an odd result caused by the `NA` value in `counts`.

    To resolve this, review the `Module1_IntrotoDataFrames` lesson, specifically about dealing with `NA` values and remind yourself about the argument `na.rm = TRUE`.
...
3.  Write a line of code that selects values less than 66 from the `counts` vector. The `NA` values will be included in the resulting vector, which is fine.

```{r}

```

4.  Create a character vector called `pet_names` which includes the following names: Mystery, Hazel, Spud, Sandy

```{r}

```

5.  Write a line of code using *sub-setting by index* to return the element "Spud" from the vector `pet_names`.

```{r}

```

6.  Now, write a line of code using *sub-setting by condition* that returns the element "Spud".
...
8.  Look at the first 6 rows of data. You can either do this by using a function or sub-setting by index.

```{r}

```

9.  Calculate the average *length* of all the cactus pads that were measured.
...
13. Run the following line of code. By looking at the code, the data frame, the output, describe what the following line of code is doing. Make sure to include each part of the code. (As a point of reference, this took me 5 sentences in the answer key). (2 points)
...
14. Create 2 new data frames: one called `ficus` with only rows that have *Opuntia ficus-indica* in the species column and one called `engel` with only rows which have *Opuntia engelmannii* in the species column. The data frames should have *all* columns.
