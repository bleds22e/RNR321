...
3. How many sites were surveyed? How many species were seen? You can either look in the environment or use code to answer these.

*Sites*: 
*Species*: 

```{r}
# optional space for code
```

4. Use the `specaccum()` function to prep the data for the species accumulation curve.
...
5.  Next, plot the species accumulation curve based on our observed data.

```{r}

```
6.  The y-axis on the plot is labeled "exact." Based on what you know about plotting species accumulation curves, what does the y-axis actually represent?
...
9. Based on the Chao estimator, how many species are estimated to be present in the community but we did not detect?
```{r}

```

10. Now, let's add our estimated species richness to our species accumulation curve. 

Remember to set the y-axis range appropriately and to run the `plot()` and `abline()` functions toegther.
...
20. Interpret the plot you just made. 

How does community 2 differ from community 1? How does the sampling (i.e., replication) differ between community 1 and community 2? 
