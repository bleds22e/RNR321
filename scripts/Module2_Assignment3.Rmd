...
1.  In 1-2 sentences, explain why we have chosen stratified random
...
2.  Create an object, `N`, which represents the total number of sample
...
3.  Add the correct values to the lines of code below to create objects
    for future use.
...
4.  Start by calculating the numerators (N~i~ \* s~i~ / square root of
    c~i~) for each stratum. Remember to use objects you've created
    rather than typing out specific numbers into the equation.
...
5.  Calculate the denominator of the equation by adding all of the
...
6.  Calculate how many sampling units we should optimally sample in each
...
7.  If we did not have the pilot study telling us about the variation of
...
Let's load our "grasshoppers.csv" data into our workspace using the
`read_csv()` function and save the data frame as an object called
`grasshoppers`.
...
We can explore the data frame to get an idea of what each row and column
represent.
...
NOTE: Open up your script from Module 2, Assignment 2---it's about to
come in handy! I would also recommend opening up the lecture slides.

8.  Before we begin calculating, stop for a moment to reflect and
...
9.  Use the code from Module 2 Assignment 2 to calculate the following
    for each stratum:
...
10. To do this, calculate the following:
    ...
    b.  variance of the estimate of the total (var_tau_hat).
        You might need to go fishing for this equation in one of the
        earlier powerpoints. (1 point)
...
#### Step 3: Calculating Overall Total Statistics

Congratulations! The hardest parts of the calculation are over (thank
goodness, right?).

Next, we will calculate the overall total (abundance) and the variance of the
estimate of the total.

11. Calculate the following:

    a.  overall total (1 point)

    ```{r}
    tau_hat <- 
    ```

    b.  variance of the estimate of the total (1 point)

    ```{r}
    var_tau_hat <- 
    ```

#### Step 4: Calculating Overall Means, etc.

Finally, we can now calculate the overall mean and variance of the estimate of the overall mean.

12. Calculate the following:

    a.  overall mean (1 point)

    ```{r}
    y_bar <- 
    ```

    b.  variance of the estimate of the overall mean (1 point)
...
Okay, that was a lot of calculating! Let's summarize and make sure we
know what just happened. We were ultimately able to calculate estimates
the parameters we were interested in: the population mean and the
population total along with the amount of uncertainty in both of those
estimates.

12. Based on our calculations above, answer the following questions with
    the **name of the R object** and its **corresponding value** for
    each calculation. (2 points)
