...
Load the `tidyverse` package. We won't be using it *too* much this time
around, but we need it a couple times.
...
*(Hint: do these numbers match your values from question 7? If not, you
...
That said, if you're feeling ambitious and want to *try* to use the
`tidyverse` and a lot of `group_by()`, `mutate()`, and `summarise()`
...
12. Calculate the following:
    ...
    b.  variance of the estimate of the overall population mean (1
        point)
