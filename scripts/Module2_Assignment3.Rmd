...
We have a sampling frame with 1000 sample units. We decide sample 100 of those units. We have our 3 strata which we have determined based on the vegetation type (we will called them A, B, and C for simplicity for the rest of the assignment).
...
Based on a pilot study done last year, we have an estimate of the amount of variation (standard deviation) for each stratum.
...
The data in `stratumA`, `stratumB`, and `stratumC` are the data collected when we surveyed each sample unit as allocated in Question 6.

Below, where you need to reference ${N}$, ${N_i}$, $n$, or $n_i$ in an equation, you can use the values you calculated in Questions 1-6 above.

## Calculating Grasshopper Abundance and Density

NOTE: Open up your previous assignments; they are likely about to come in handy! I would also recommend opening up the lecture slides. Remember to download the slide decks to see all of the equations or use the PDF versions.

8.  Before we begin calculating, stop for a moment to reflect and predict. For stratified sampling, we have a lot of steps to take before we get to our ultimate goal: estimates of density and abundance of the entire grasshopper population, along with the amount of uncertainty in those estimates.

    Take a minute to skim through the lecture slides and remind yourself of the process of how we go about calculating the estimates of population parameters and the amount of uncertainty in those estimates. Describe (either in sentences or in bullet points) what the best way to proceed through our calculations will be.
...
9.  Calculate the following for each stratum:
    ...
    a.  average grasshoppers per stratum (1 point)
...
10. To do this, calculate the following:
    ...
    a.  Variance of the estimate of the mean per stratum (`var_y_bar`).
        ...
        NOTE! While they are normally interchangable, in this specific instance please use the sample variances calculated in Q9c instead of the standard deviations from between Q3 and Q4 above. (2 points)
    ...
    b.  Variance of the estimate of the total abundance (`var_tau_hat`). (1 point)
...
### Step 3: Calculating the Overall Total Population Estimate and Uncertainty
