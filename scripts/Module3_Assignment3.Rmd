...
First things first, let's load our package.
...
1.  First, plot the distribution of detection distances. Does is look how you expected?
...
3.  Use unmarked function `formatDistData` to count the number of saguaros in each distance bin for each transect. Save this as an object called `sag_data`. 
...
4.  Using the `unique()` function, make a data frame with the transect length for each group (i.e., survey effort). Call this `transect_saguaro`. 

*Make sure you are referencing the correct columns in the square brackets!*
...
-   What is the estimated density of saguaros (include units!)? What about the confidence intervals? 
...
12. This time around, set truncation distance to 25m; save this as an object called `trunc_oco`. Then create your distance vector (`dinstance_bins_oco`) and set the cut points at every 5 meters.
...
14. Make your new data frame with the transect lengths. Call this `transect_oco`.
