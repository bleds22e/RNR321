...
```{r}
# Model p(.)psi(.) -- psi constant across sites, det prob constant across survey occasion
...
# coef() function retuns model coefficients (estimates)
# plogis() coverts the values from a logistic scale to probabilities

summary(pdot_psidot)

# For model: p(.)psi(.)
plogis(coef(pdot_psidot))  # estimates of psi (occupancy) and p (detective probability)
